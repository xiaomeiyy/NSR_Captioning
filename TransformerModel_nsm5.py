...
#         y = x + self.dropout(self.normlayer[0](x_org)) + self.dropout(self.normlayer[2](rel_vec))
...
class RelationAttention(nn.Module):
    def __init__(self, src_attn, rel_glove, top_num, d_model, dropout=0.1):
        super(RelationAttention, self).__init__()
        self.rel_glove, self.d_model, self.top_num = rel_glove, d_model, top_num       
        self.glove_embed = nn.Linear(rel_glove.shape[1], d_model)
    ...
    def forward(self, A_rel, rel_scores, att_feats, att_mask):
        ...
        ## get semantic reps by word embeddings
        rel_vecs = torch.mm(rel_scores.view(-1,num_rel_cls),self.glove_embed(self.rel_glove)).view(bs,num_node*num_node,-1)
        rel_vecs = self.linears[0](rel_vecs)
        ...
        rel_reps = self.src_attn(att_feats, pe_rel_topk, pe_rel_topk)  # batch * rnn_size
        
        ## refresh  A                         
        node_pre = self.feat_emb(torch.cat([rel_reps, att_feats],dim=2))
        A_added = torch.matmul(node_pre, node_pre.transpose(1, 2))
        A_new = A_added + A_rel
        
        return A_new, rel_reps
...
#         position = LSTMEncoding(self.opt.vocab_glove, d_model, dropout)
        ...
        rel_attn = RelationAttention(c(attn), self.opt.rel_glove, self.opt.top_num, d_model, dropout)
